...
- [ADAS visualize service](#adas-visualize-service)
  - [Build for PC](#build-adas_visualize_service-for-pc)
  ...
  - [Build for the target board](#build-adas_visualize_service-for-the-target-board)
    ...
    - [Build and Run (Step by Step)](#build-and-run-step-by-step-1)
      ...
      - [3. After compiled, source before run adas_visualize_service](#3-after-compiled-source-before-run-adas_visualize_service)
...
```
./script/run_adas.sh --launch=visualize
```
...
make -j4
```
#### 3. After compiled, source before run the adas_service
```
source ../script/setup_adas_service_runtime_env.bash
./pc_build/adas_visualize_service/adas_visualize_service --src_type=udp
```
...
Run commands on target board for adas_visualize_service
```
./script/run_adas.sh --launch=visualize     # Run on IVI board with DDS mode
```
...
#### 3. After compiled, source before run adas_visualize_service
...
# Run adas_visualize_service
./build/adas_visualize_service/adas_visualize_service --src_type=udp
```
