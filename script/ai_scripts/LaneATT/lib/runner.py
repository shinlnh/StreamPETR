...
class Runner:
    ...
    def eval(self, epoch, on_val=False, save_predictions=False):
        ...
        with torch.no_grad():
            for idx, (images, targets, _) in enumerate(tqdm(dataloader)):
                images = images.to(self.device)
                ...
                if self.view:
                    ...
                    img_pred, fp, fn = dataloader.dataset.draw_annotation(idx, img=img, pred=prediction[0])
                    if self.view == 'mistakes' and fp == 0 and fn == 0:
                        continue
                    if 0: # enable to visualize matched anchors
                        targets = targets.to(self.device)
                        matched_idx, _, _, _ = match_proposals_with_targets(model, model.anchors, targets[0], t_pos=15., t_neg=20.)
                        matched_anchors = model.anchors[matched_idx]
                        matched_anchors[:, 4] = len(model.anchor_ys) * 2/3
                        anchor_extended_cls = torch.ones((len(matched_anchors), model.n_class), device= self.device)
                        matched_anchors = torch.cat((anchor_extended_cls, matched_anchors[:, 2:]), dim= -1)
                        matched_anchors = model.decode([[matched_anchors, 0, 0, 0]], as_lanes=True)
                        img_anchor, _, _ = dataloader.dataset.draw_annotation(idx, img=img, pred=matched_anchors[0])
                        img_pred = cv2.vconcat([img_pred, img_anchor])
    ...
    def export(self, epoch, output_path):
        model = self.cfg.get_model()
        model_path = self.exp.get_checkpoint_path(epoch)
        self.logger.info('Loading model %s', model_path)
        model.load_state_dict(self.exp.get_epoch_model(epoch))
        ...
        model.eval()
        self.logger.info('Exporting to ONNX file: %s', output_path)
        with torch.no_grad():
            model.export = True
            model = model.to()
            images = torch.zeros((1, 3, model.img_h, model.img_w)).to(torch.device('cpu'))
