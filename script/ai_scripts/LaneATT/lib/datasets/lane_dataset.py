...
class LaneDataset(Dataset):
    ...
    def transform_annotation(self, anno, img_wh=None):
        ...
        lanes_type = anno['types'] if 'types' in anno else np.ones(len(old_lanes), dtype=np.float32)

        # removing lanes with less than 2 points
        mask = [len(lane) > 1 for lane in old_lanes]
        lanes_type = [lane_type for i, lane_type in enumerate(lanes_type) if mask[i]]
        old_lanes = [lane for i, lane in enumerate(old_lanes) if mask[i]]
        ...
        for lane_idx, lane in enumerate(old_lanes):
            ...
            lanes[lane_idx, 0] = lanes_type[lane_idx]
            lanes[lane_idx, 1] = 1
    ...
    def __getitem__(self, idx):
        ...
        for i in range(30):
            ...
            if 'types' in item['old_anno']:
                new_anno['types'] = item['old_anno']['types']
            try:
