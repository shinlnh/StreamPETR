...
def parse_args():
    ...
    parser.add_argument('--section', default='monolith', help='section can be either extract_img_feat or pts_head_memory')
...
def main():
    ...
    class TrtPtsHeadContainer(torch.nn.Module):
        def __init__(self, section, mod, *args, **kwargs) -> None:
            super().__init__(*args, **kwargs)
            self.section = section
            self.mod = mod

        def _save_pseudo_references(self):
            ...
            pseudo_reference_points.detach().cpu().numpy().astype(np.float32).tofile("pseudo_reference_points.bin")

        def _save_position_encoder(self):
            head = self.mod.pts_bbox_head
            head.position_encoder[0].weight.detach().cpu().numpy().T.astype(np.float32).tofile("position_encoder_w1.bin")
            head.position_encoder[0].bias.detach().cpu().numpy().astype(np.float32).tofile("position_encoder_b1.bin")
            head.position_encoder[2].weight.detach().cpu().numpy().T.astype(np.float32).tofile("position_encoder_w2.bin")
            head.position_encoder[2].bias.detach().cpu().numpy().astype(np.float32).tofile("position_encoder_b2.bin")

        def _gather_new_memory(self, all_cls_scores, all_bbox_preds, outs_dec):
            rec_reference_points = all_bbox_preds[..., :3][-1]
            rec_velo = all_bbox_preds[..., -2:][-1]
            rec_memory = outs_dec[-1]
            rec_score = all_cls_scores[-1].topk(1, dim=-1).values[..., 0:1]
            ...
            return rec_reference_points, rec_memory, rec_velo
        ...
        def forward(self, x, pos_embed, cone, 
            ...
            head.memory_velo = memory_velo
            
            memory, pos_embed, reference_points, tgt, temp_memory, temp_pos, \
            query_pos, query_pos_in, outs_dec, all_cls_scores, all_bbox_preds, rec_ego_pose = \
                self._pts_head(x, pos_embed, cone)
        
            # memory update after head
            rec_reference_points, rec_memory, rec_velo = self._gather_new_memory(all_cls_scores,
                                                                                 all_bbox_preds,
                                                                                 outs_dec)

            return all_cls_scores, all_bbox_preds, \
                rec_reference_points, rec_memory, rec_velo, \
                memory, reference_points, tgt, temp_memory, temp_pos, query_pos, query_pos_in, outs_dec
                # head.memory_embedding, head.memory_reference_point, head.memory_timestamp, head.memory_egopose, head.memory_velo,
                
    model.eval()
    model = model.float()

    if args.section not in ["extract_img_feat", "pts_head_memory", "monolith"]:
    ...
    if args.section == "extract_img_feat":        
        ...
        tm = TrtPtsHeadContainer(args.section, model.module)
        tm._save_position_encoder()
        tm._save_pseudo_references()
        mem_len = tm.mod.pts_bbox_head.memory_len
        arrs = [
            ...
            # torch.from_numpy(np.random.uniform(-0.5, 0.5, size=(1, 4, 4))).float(),  
            torch.from_numpy(np.random.uniform(-0.5, 0.5, size=(1, mem_len, 256))).float(),
            torch.from_numpy(np.random.uniform(-0.5, 0.5, size=(1, mem_len, 3))).float(),
            torch.from_numpy(np.random.uniform(-0.5, 0.5, size=(1, mem_len, 1))).float(),
            torch.from_numpy(np.random.uniform(-0.5, 0.5, size=(1, mem_len, 4, 4))).float(),
            torch.from_numpy(np.random.uniform(-0.5, 0.5, size=(1, mem_len, 2))).float(),
        ]
        input_names=["img_feats", "pos_embed", "cone",
                     "memory_embedding",
        ...
        if args.section == "monolith":
            input_names[0] = "img"
            arrs[0] = torch.from_numpy(np.random.uniform(-0.5, 0.5, size=(1, 6, 3, 256, 704))).float()
    ...
    with torch.no_grad():
        torch.onnx.export(
            ...
            opset_version=17,  # set to 13 if TRT does not support
            verbose=False)
