...
cv::Rect get_rect(cv::Mat& img, const float bbox[4]) {
  float l, r, t, b;
  float r_w = OD_INPUT_W / (img.cols * 1.0);
  float r_h = OD_INPUT_H / (img.rows * 1.0);
  if (r_h > r_w) {
    l = bbox[0] - bbox[2] / 2.f;
    r = bbox[0] + bbox[2] / 2.f;
    t = bbox[1] - bbox[3] / 2.f - (OD_INPUT_H - r_w * img.rows) / 2;
    b = bbox[1] + bbox[3] / 2.f - (OD_INPUT_H - r_w * img.rows) / 2;
    l = l / r_w;
    r = r / r_w;
    t = t / r_w;
    b = b / r_w;
  } else {
    l = bbox[0] - bbox[2] / 2.f - (OD_INPUT_W - r_h * img.cols) / 2;
    r = bbox[0] + bbox[2] / 2.f - (OD_INPUT_W - r_h * img.cols) / 2;
    t = bbox[1] - bbox[3] / 2.f;
    b = bbox[1] + bbox[3] / 2.f;
    l = l / r_h;
    r = r / r_h;
...
void nms(std::vector<Detection>& res, float* output, float conf_thresh, float nms_thresh) {
  ...
  for (int i = 0; i < output[0] && i < OD_MAX_NUM_OUTPUT_BBOX; i++) {
...
cv::Rect get_downscale_rect(const float bbox[4], float scale) {
