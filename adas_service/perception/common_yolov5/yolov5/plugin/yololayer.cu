...
YoloLayerPlugin::YoloLayerPlugin(int classCount, int netWidth, int netHeight, int maxOut, bool is_segmentation, const std::vector<YoloKernel>& vYoloKernel) {
  ...
  size_t AnchorLen = sizeof(float)* OD_NUM_ANCHOR * 2;
...
__global__ void CalDetection(const float *input, float *output, int noElements,
    ...
    int yoloHeight, const float anchors[OD_NUM_ANCHOR * 2], int classes, int outputElem, bool is_segmentation) {
  ...
  const float* curInput = input + bnIdx * (info_len_i * total_grid * OD_NUM_ANCHOR);

  for (int k = 0; k < OD_NUM_ANCHOR; ++k) {
    float box_prob = Logist(curInput[idx + k * info_len_i * total_grid + 4 * total_grid]);
    if (box_prob < OD_IGNORE_THRESH) continue;
    int class_id = 0;
