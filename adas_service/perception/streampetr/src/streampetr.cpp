...
void StreamPETR::_calc_img_lidar_transform()
{
    ...
    std::cout << this->cam2imgs[0] << std::endl;
    std::cout << this->ego2cams[0] << std::endl;
    std::cout << this->ida_mats[0] << std::endl;
    std::cout << this->lidar2imgs[0] << std::endl;
    throw std::runtime_error("");
}
...
StreamPETR::Output StreamPETR::infer(Input input)
{
    ...
    // size_t size = input.rawImgs[0].cols * input.rawImgs[0].rows * 3;
    // u_char min = *std::min_element(input.rawImgs[0].data, input.rawImgs[0].data + size);
    // u_char max = *std::max_element(input.rawImgs[0].data, input.rawImgs[0].data + size);
    // uint64_t sum = std::accumulate(input.rawImgs[0].data, input.rawImgs[0].data + size, 0);
    // float ave = (double)sum / size;
    // INFO("%u %u %f", min, max, ave);
    ...
    // float min = *std::min_element(img_data.begin(), img_data.end());
    // float max = *std::max_element(img_data.begin(), img_data.end());
    // double sum = std::accumulate(img_data.begin(), img_data.end(), 0.0);
    // float ave = sum/img_data.size();
    // INFO("%f %f %f", min, max, ave);
    ...
    // auto memory_embedding = this->detector_->bindings["memory_embedding"]->cpu();
    // INFO("%f %f %f",
    //     *std::min_element(memory_embedding.begin(), memory_embedding.end()),
    //     *std::max_element(memory_embedding.begin(), memory_embedding.end()),
    //     std::accumulate(memory_embedding.begin(), memory_embedding.end(), 0.0) / memory_embedding.size()
    // );

    // auto memory_reference_point = this->detector_->bindings["memory_reference_point"]->cpu();
    // INFO("%f %f %f",
    //     *std::min_element(memory_reference_point.begin(), memory_reference_point.end()),
    //     *std::max_element(memory_reference_point.begin(), memory_reference_point.end()),
    //     std::accumulate(memory_reference_point.begin(), memory_reference_point.end(), 0.0) / memory_reference_point.size()
    // );

    // auto memory_velo = this->detector_->bindings["memory_velo"]->cpu();
    // INFO("%f %f %f",
    //     *std::min_element(memory_velo.begin(), memory_velo.end()),
    //     *std::max_element(memory_velo.begin(), memory_velo.end()),
    //     std::accumulate(memory_velo.begin(), memory_velo.end(), 0.0) / memory_velo.size()
    // );
