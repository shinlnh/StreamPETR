...
void FitCurve(Point2 *d, int nPts, double error, int coarseAlgo);
static void FitCubic(Point2	*d, int	first, int last, Vector2 tHat1, Vector2 tHat2, double error, int coarseAlgo);
...
Point2 globalBezierCurve[MAXPOINTS];
int globalIndex = 0; // Used to index the return points array

void DrawBezierCurve(int n, BezierCurve curve)
{
    // Copy the curve array to the global array
    for (int i = 0; i <= n; i++)
    {
        globalBezierCurve[globalIndex].x = curve[i].x;
        globalBezierCurve[globalIndex].y = curve[i].y;
        globalIndex++;
    }
}
...
{
    FitCurve(points, numPoints, error, coarse);		/*  Fit the Bezier curves */
    globalBezierCurve[globalIndex].x = END_OF_LIST;
    globalBezierCurve[globalIndex].y = END_OF_LIST;
    globalIndex = 0;
    return globalBezierCurve;
}
...
void FitCurve(Point2 *d, int nPts, double error, int coarseAlgo)
{
    ...
    FitCubic(d, 0, nPts - 1, tHat1, tHat2, error, coarseAlgo);
}
...
static void FitCubic(Point2	*d, int	first, int last, Vector2 tHat1, Vector2 tHat2, double error, int coarseAlgo)
{
    ...
    if (nPts == 2) {
        double dist = V2DistanceBetween2Points(&d[last], &d[first]) / (coarseAlgo / 100);
    ...
    FitCubic(d, first, splitPoint, tHat1, tHatCenter, error, coarseAlgo);
    V2Negate(&tHatCenter);
    FitCubic(d, splitPoint, last, tHatCenter, tHat2, error, coarseAlgo);
}
...
static Vector2 ComputeRightTangent(Point2 *d, int end)		/*  Index to "right" end of region */
{
    Vector2	tHat2;
